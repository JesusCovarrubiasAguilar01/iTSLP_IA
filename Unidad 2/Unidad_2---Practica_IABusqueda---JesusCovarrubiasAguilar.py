...
class Nodo:
    ...
    def mover(self, direccion):
        ...
        if direccion == "arriba" and ind not in [6, 7, 8]:
            estado[ind], estado[ind + 3] = estado[ind + 3], estado[ind]
        elif direccion == "abajo" and ind not in [0, 1, 2]:
            estado[ind], estado[ind - 3] = estado[ind - 3], estado[ind]
        elif direccion == "derecha" and ind not in [0, 3, 6]:
            estado[ind], estado[ind - 1] = estado[ind - 1], estado[ind]
    ...
    def encontrar_sucesores(self):
        ...
        for movimiento in movimientos:
            ...
            if sucesor:
                sucesores.append(Nodo(sucesor, self, movimiento, self.profundidad + 1, calcular_heurisitica(sucesor)))
        return sucesores
...
def calcular_heurisitica(estado):
    ...
    piezas_correctas = sum(1 for a, b in zip(estado, correcto) if a == b)
...
def main():
    ...
    nodo_inicial = Nodo(estado_inicial, None, None, 0, calcular_heurisitica(estado_inicial))
    nodo_inicial.imprimir_nodo()
    coordenadas_7 = nodo_inicial.coordenadas_numero(7)  # Cambio aquí para obtener las coordenadas del número 7
    if coordenadas_7:
        print(f"El número 7 está ubicado en {coordenadas_7}")  # Cambio aquí para mostrar las coordenadas del número 7
    ...
    if nodos_camino:
        ...
        if imprimir_camino.lower() == "s":
            ...
            for nodo in nodos_camino:
                ...
                print("Estado actual:")
                nodo.imprimir_nodo()
                coordenadas_7 = nodo.coordenadas_numero(7)  # Cambio aquí para obtener las coordenadas del número 7
                if coordenadas_7:
                    print(f"El número 7 está ubicado en {coordenadas_7}")  # Cambio aquí para mostrar las coordenadas del número 7
